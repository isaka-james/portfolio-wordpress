	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('portfolio-canvas-list-item'); ?>>
		<div class="grid-blog-item p-3">
			...
			<div class="grid-deatls pb-3">
				<div class="row pt-3">
					<?php if ('post' === get_post_type() && !empty($portfolio_canvas_category)) : ?>
						<div class="col-md-6 grid-meta">
							<a class="blog-categrory" href="<?php echo esc_url(get_category_link($portfolio_canvas_category)); ?>"><?php echo esc_html($portfolio_canvas_category->name); ?></a>
						</div>
					<?php endif; ?>
				...
				<a class="read-more-btn" href="<?php the_permalink(); ?>"><?php esc_html_e('Read More ', 'portfolio-canvas'); ?> <i class="fas fa-arrow-right"></i></a>
			</div>
		</div>
